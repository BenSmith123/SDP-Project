...
function check_collision(_move_x, _move_y) 
{
	...
	if (vel_y >= 0)
	{
        var _platform = instance_place(x + _move_x, y + _move_y, obj_block_jump_through)
        if (_platform != noone)
        {
	...
	var _left_top = tilemap_get_at_pixel(obj_controller.collision_tilemap, bbox_left + _move_x, bbox_top + _move_y);

	// This checks for tile collision at the top-right corner of the instance's mask
	var _right_top = tilemap_get_at_pixel(obj_controller.collision_tilemap, bbox_right + _move_x, bbox_top + _move_y);

	// This checks for tile collision at the bottom-right corner of the instance's mask
	var _right_bottom = tilemap_get_at_pixel(obj_controller.collision_tilemap, bbox_right + _move_x, bbox_bottom + _move_y);

	// This checks for tile collision at the bottom-left corner of the instance's mask
	var _left_bottom = tilemap_get_at_pixel(obj_controller.collision_tilemap, bbox_left + _move_x, bbox_bottom + _move_y);
